...
PROCESS_THREAD(init_system_process, ev, data) {
  ...
  etimer_set(&time_poll,2*CLOCK_SECOND);
  ...
  while(1) {
    ...
    temp_sensor_value = (50);
    humid_sensor_value = (100);
    ...
    if (led_on_off ==1) { 
      ...
      printf("\nMessage sent: topic /led_control payload:red_on");
    
      //mqtt_sn_pub("/led_control","green_on",true,0);
      //mqtt_sn_pub("/led_control","green_off",true,0);
      //mqtt_sn_pub("/led_control","blue_on",true,0);
      //mqtt_sn_pub("/led_control","blue_off",true,0);
    }
    else {   
      mqtt_sn_pub("/led_control","red_off",true,0);
      printf("\nMessage sent: topic /led_control payload:red_off");
    }
  }
